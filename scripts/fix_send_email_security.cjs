...
const replacement = `  app.post('/api/send-email', async (req, res) => {
    ...
    try {
      ...
      if (pdfBase64) mail.attachments = [{ filename: \\`Al_Andalos_Dossier_\\${(studentName || 'Student').replace(/[\\s]+/g, '_')}.pdf\\`, content: Buffer.from(String(pdfBase64).replace(/^data:application\\/pdf;base64,/, ''), 'base64'), contentType: 'application/pdf' }];
