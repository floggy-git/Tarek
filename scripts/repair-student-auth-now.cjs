...
if (s.includes(oldRegister)) s = s.replace(oldRegister, newRegister);
s = s.replace(/\s*password:\s*hashedPassword,\n/g, '\n');
s = s.replace(/bcrypt\.[A-Za-z]+\([^\n]*\)/g, "''");
s = s.replace(/import bcrypt from 'bcryptjs';\n?/g, '');
s = s.replace(/student123/g, '');
if (s.includes('bcrypt.compareSync') || s.includes('bcrypt.hashSync') || s.includes('loginPassword === storedPassword') || s.includes('password: hashedPassword')) throw new Error('legacy password verification remains');
