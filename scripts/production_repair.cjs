...
// The two fields intentionally share the same class. Replace that exact class
// once per occurrence; no broad scan, no JSX parsing, no watcher churn.
{
  ...
  s = s.replace(timeInputClass, repairedClass);
  s = s.replace(timeInputClass, repairedClass);
